...
type clientParams struct {
	fx.In
	ClientServer    jsonrpc.Client `name:"server"`
...
func TestClients(t *testing.T) {
	...
	app := testapp.New(
		t,
		withDummyEndpoints(),
		fx.Provide(endpoints.NewEndpointProvider),
	...
	require.NotNil(params.ClientProxy)
	require.Equal(1, len(params.ClientProxy.GetEndpointProvider().GetAllEndpoints()))
	require.Equal("node_url", params.ClientProxy.GetEndpointProvider().GetAllEndpoints()[0].Url)
	require.NotNil(params.ClientValidator)
	require.Equal(1, len(params.ClientValidator.GetEndpointProvider().GetAllEndpoints()))
	require.Equal("validator_url", params.ClientValidator.GetEndpointProvider().GetAllEndpoints()[0].Url)
}
...
func TestCall_URLError(t *testing.T) {
	...
	httpClient.EXPECT().Do(gomock.Any()).Return(nil, urlError).Times(maxRetryAttempts)
...
func TestBatchCall_RetryNullResponse(t *testing.T) {
	...
	attempts := 0
	httpClient.EXPECT().Do(gomock.Any()).Times(maxRetryAttempts).
	...
	batchResponse, err := client.BatchCall(context.Background(),
		...
		batchParams)
	require.NoError(err)
	require.Equal(maxRetryAttempts, attempts)
...
func TestNullResponse(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(test.expected, response.IsNullOrEmpty())
		})
	}
}
...
func TestRPCError(t *testing.T) {
	require := testutil.Require(t)
	err := jsonrpc.NewRPCError(-32999, "mock error")
	require.Equal(err.Error(), "mock error")
	require.NotContains(err.Error(), "-32999")

	verbose := fmt.Sprintf("%+v", err)
	require.Contains(verbose, "RPCError -32999: mock error")
	require.NotContains(verbose, "client_test.go")
}

func TestRPCErrorWithCause(t *testing.T) {
	require := testutil.Require(t)
	err := jsonrpc.NewRPCError(-32999, "mock error").
		WithCause(xerrors.New("something else"))
	require.Equal(err.Error(), "mock error")
	require.NotContains(err.Error(), "-32999")

	verbose := fmt.Sprintf("%+v", err)
	require.Contains(verbose, "RPCError -32999: mock error")
	require.Contains(verbose, "Caused by: something else")
	require.Contains(verbose, "client_test.go")
}
...
func TestRPCErrorWithData(t *testing.T) {
	...
	require.Equal(json.RawMessage(`"foo"`), err.ErrorData())
}
...
func withDummyEndpoints() fx.Option {
	...
	dummyValidatorEndpoints := []config.Endpoint{
		{
			Name:   "validator_node",
			Url:    "validator_url",
			Weight: 1,
	...
	cfg.Chain.Client = config.ClientConfig{
		Primary: config.EndpointGroup{
			Endpoints: dummyEndpoints,
		},
		Validator: config.EndpointGroup{
			Endpoints: dummyValidatorEndpoints,
		},
