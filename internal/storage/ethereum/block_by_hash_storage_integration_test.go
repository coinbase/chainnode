...
type blockByHashStorageIntegrationTestSuite struct {
	suite.Suite
...
func (s *blockByHashStorageIntegrationTestSuite) SetupTest() {
	...
	var storage BlockByHashStorage
	s.app = testapp.New(
		s.T(),
...
func (s *blockByHashStorageIntegrationTestSuite) TearDownTest() {
	if s.app != nil {
		s.app.Close()
	}
}

func TestIntegrationBlobByHashStorageTestSuite(t *testing.T) {
	suite.Run(t, new(blockByHashStorageIntegrationTestSuite))
}

func (s *blockByHashStorageIntegrationTestSuite) TestPersistAndGetBlockByHash() {
	require := testutil.Require(s.T())
	...
	err := s.storage.PersistBlockByHash(ctx, expectedFirstBlock)
	require.NoError(err)

	persisted, err := s.storage.GetBlockByHash(ctx, tag, hash, sequence)
	require.NoError(err)
	...
	persisted, err = s.storage.GetBlockByHash(ctx, tag, hash, sequence+1)
	require.NoError(err)
	require.Equal(expectedFirstBlock, persisted)
	...
	err = s.storage.PersistBlockByHash(ctx, expectedSecondBlock)
	require.NoError(err)
	...
	err = s.storage.PersistBlockByHash(ctx, oldBlock)
	require.NoError(err)

	read, err := s.storage.GetBlockByHash(ctx, tag, hash, sequence)
	require.NoError(err)
	// still gets the result from the second attempt
	require.Equal(persisted, read)
}

func (s *blockByHashStorageIntegrationTestSuite) TestPersistAndGetBlockByHash_LargeBlock() {
	require := testutil.Require(s.T())
...
func (s *blockByHashStorageIntegrationTestSuite) TestGetBlockByHash_NotExists() {
	require := testutil.Require(s.T())
	...
	block, err := s.storage.GetBlockByHash(ctx, tag, hash, sequence)
	require.Error(err)
	require.True(xerrors.Is(err, internal.ErrItemNotFound))
	require.Nil(block)
}

func (s *blockByHashStorageIntegrationTestSuite) TestPersistBlockByHash_NilBlock() {
	require := testutil.Require(s.T())

	err := s.storage.PersistBlockByHash(context.Background(), nil)
	require.Error(err)
}
