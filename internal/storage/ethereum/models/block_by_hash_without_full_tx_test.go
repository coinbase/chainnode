...
func TestBlockByHashWithoutFullTx(t *testing.T) {
	...
	require.Equal("1#blocks-by-hash-without-full-tx#0x1", MakeBlockByHashWithoutFullTxPartitionKey(tag, hash))
	require.Equal("000000000000022b", MakeBlockByHashWithoutFullTxSortKey(sequence))
	...
	for _, blockTime := range blockTimes {
		...
		entry, err := MakeBlockByHashWithoutFullTxDDBEntry(block)
		require.NoError(err)
		...
		require.NoError(err)
		require.Equal("1/blocks-by-hash-without-full-tx/0x1/555", objectKey)

		pk, err := entry.GetPartitionKey()
		require.NoError(err)
		require.Equal("1#blocks-by-hash-without-full-tx#0x1", pk)
