...
func TestTraceByHash(t *testing.T) {
	...
	require.Equal("1#traces-by-hash#0xo123", MakeTraceByHashPartitionKey(tag, hash))
	require.Equal("000000000000022b", MakeTraceByHashSortKey(sequence))
	...
	for _, blockTime := range blockTimes {
		...
		entry, err := MakeTraceByHashDDBEntry(trace)
		require.NoError(err)
		...
		require.NoError(err)
		require.Equal("1/traces-by-hash/0xo123/555", objectKey)

		pk, err := entry.GetPartitionKey()
		require.NoError(err)
		require.Equal("1#traces-by-hash#0xo123", pk)
