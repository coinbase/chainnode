package ethereum

import (
	"context"
	"testing"
	"time"

	"github.com/golang/mock/gomock"
	"github.com/stretchr/testify/suite"
	"go.uber.org/fx"
	"golang.org/x/xerrors"

	"github.com/coinbase/chainnode/internal/api"
	xapi "github.com/coinbase/chainnode/internal/api/ethereum"
	"github.com/coinbase/chainnode/internal/storage/collection"
	cmocks "github.com/coinbase/chainnode/internal/storage/collection/mocks"
	"github.com/coinbase/chainnode/internal/storage/ethereum/models"
	"github.com/coinbase/chainnode/internal/utils/compression"
	"github.com/coinbase/chainnode/internal/utils/testapp"
	"github.com/coinbase/chainnode/internal/utils/testutil"
)

const (
	traceByHashPartitionKeyFixture = "1#traces-by-hash#0x123"
	traceByHashSortKeyFixture      = "0000000000003018"
	traceByHashTagFixture          = uint32(1)
	traceByHashSequenceFixture     = api.Sequence(12312)
	traceByHashHeightFixture       = uint64(123456)
	traceByHashHashFixture         = "0x123"
	traceByHashBlockTimeFixture    = "2020-11-24T16:07:21Z"
)

type TraceByHashStorageTestSuite struct {
	suite.Suite
	ctrl              *gomock.Controller
	collectionStorage *cmocks.MockCollectionStorage
	storage           TraceByHashStorage
	app               testapp.TestApp
	blockTimeFixture  time.Time
}

func TestTraceByHashStorageTestSuite(t *testing.T) {
	suite.Run(t, new(TraceByHashStorageTestSuite))
}

func (s *TraceByHashStorageTestSuite) SetupTest() {
	s.ctrl = gomock.NewController(s.T())
	s.collectionStorage = cmocks.NewMockCollectionStorage(s.ctrl)
	s.collectionStorage.EXPECT().
		WithCollection(xapi.CollectionTracesByHash).
		Return(s.collectionStorage)
	logsStorage := cmocks.NewMockCollectionStorage(s.ctrl)

	s.app = testapp.New(
		s.T(),
		fx.Provide(newTraceByHashStorage),
		fx.Provide(fx.Annotated{Name: "collection", Target: func() collection.CollectionStorage { return s.collectionStorage }}),
		fx.Provide(fx.Annotated{Name: "logs", Target: func() collection.CollectionStorage { return logsStorage }}),
		fx.Populate(&s.storage),
	)
	s.blockTimeFixture = testutil.MustTime(traceByHashBlockTimeFixture)
}

func (s *TraceByHashStorageTestSuite) TearDownTest() {
	s.app.Close()
	s.ctrl.Finish()
}

func (s *TraceByHashStorageTestSuite) TestPersistTrace() {
	require := testutil.Require(s.T())
	ctx := context.Background()

	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)

	trace := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Sequence:  traceByHashSequenceFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
	}

	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithData(compressed),
		Hash:      traceByHashHashFixture,
		Height:    traceByHashHeightFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	s.collectionStorage.EXPECT().
		UploadToBlobStorage(gomock.Any(), gomock.Any(), false).Return(nil)
	s.collectionStorage.EXPECT().WriteItem(gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, item interface{}) error {
			actual, ok := item.(*models.TraceByHashDDBEntry)
			require.True(ok)
			testutil.MustTime(actual.UpdatedAt)
			entry.UpdatedAt = actual.UpdatedAt
			require.Equal(entry, actual)
			return nil
		})

	err = s.storage.PersistTraceByHash(ctx, trace)
	require.NoError(err)
}

func (s *TraceByHashStorageTestSuite) TestPersistTrace_NoBlockTime() {
	require := testutil.Require(s.T())
	ctx := context.Background()

	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)

	trace := &xapi.Trace{
		Tag:      traceByHashTagFixture,
		Height:   traceByHashHeightFixture,
		Hash:     traceByHashHashFixture,
		Sequence: traceByHashSequenceFixture,
		Data:     data,
	}

	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithData(compressed),
		Hash:   traceByHashHashFixture,
		Height: traceByHashHeightFixture,
	}

	s.collectionStorage.EXPECT().
		UploadToBlobStorage(gomock.Any(), gomock.Any(), false).Return(nil)
	s.collectionStorage.EXPECT().WriteItem(gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, item interface{}) error {
			actual, ok := item.(*models.TraceByHashDDBEntry)
			require.True(ok)
			testutil.MustTime(actual.UpdatedAt)
			entry.UpdatedAt = actual.UpdatedAt
			require.Equal(entry, actual)
			return nil
		})

	err = s.storage.PersistTraceByHash(ctx, trace)
	require.NoError(err)
}

func (s *TraceByHashStorageTestSuite) TestPersistTrace_LargeFile() {
	require := testutil.Require(s.T())
	ctx := context.Background()

	data := []byte("asdqwd")
	objectKey := "1/traces-by-hash/123456/0x123"

	trace := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Sequence:  traceByHashSequenceFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
	}

	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithObjectKey(objectKey),
		Hash:      traceByHashHashFixture,
		Height:    traceByHashHeightFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	s.collectionStorage.EXPECT().
		UploadToBlobStorage(gomock.Any(), gomock.Any(), false).
		DoAndReturn(func(ctx context.Context, entry collection.Item, enforceUpload bool) error {
			err := entry.SetData(nil)
			require.NoError(err)
			err = entry.SetObjectKey(objectKey)
			require.NoError(err)
			return nil
		})
	s.collectionStorage.EXPECT().WriteItem(gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, item interface{}) error {
			actual, ok := item.(*models.TraceByHashDDBEntry)
			require.True(ok)
			testutil.MustTime(actual.UpdatedAt)
			entry.UpdatedAt = actual.UpdatedAt
			require.Equal(entry, actual)
			return nil
		})

	err := s.storage.PersistTraceByHash(ctx, trace)
	require.NoError(err)
}

func (s *TraceByHashStorageTestSuite) TestPersistTrace_UploadEnforced() {
	require := testutil.Require(s.T())
	ctx := context.Background()

	data := []byte("asdqwd")
	objectKey := "1/traces-by-hash/123456/0x123"

	s.app.Config().Storage.TraceUploadEnforced = true
	trace := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Sequence:  traceByHashSequenceFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
	}

	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithObjectKey(objectKey),
		Hash:      traceByHashHashFixture,
		Height:    traceByHashHeightFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	s.collectionStorage.EXPECT().
		UploadToBlobStorage(gomock.Any(), gomock.Any(), true).
		DoAndReturn(func(ctx context.Context, entry collection.Item, enforceUpload bool) error {
			err := entry.SetData(nil)
			require.NoError(err)
			err = entry.SetObjectKey(objectKey)
			require.NoError(err)
			return nil
		})
	s.collectionStorage.EXPECT().WriteItem(gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, item interface{}) error {
			actual, ok := item.(*models.TraceByHashDDBEntry)
			require.True(ok)
			testutil.MustTime(actual.UpdatedAt)
			entry.UpdatedAt = actual.UpdatedAt
			require.Equal(entry, actual)
			return nil
		})

	err := s.storage.PersistTraceByHash(ctx, trace)
	require.NoError(err)
}

func (s *TraceByHashStorageTestSuite) TestPersistTrace_LargeFileUploadError() {
	require := testutil.Require(s.T())
	ctx := context.Background()

	data := []byte("asdqwd")

	trace := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Sequence:  traceByHashSequenceFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
	}

	s.collectionStorage.EXPECT().
		UploadToBlobStorage(gomock.Any(), gomock.Any(), false).
		Return(xerrors.Errorf("failed to upload"))

	err := s.storage.PersistTraceByHash(ctx, trace)
	require.Error(err)
}

func (s *TraceByHashStorageTestSuite) TestGetTraces() {
	require := testutil.Require(s.T())

	ctx := context.Background()
	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)
	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithData(compressed),
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	expectedTraces := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Sequence:  traceByHashSequenceFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
		UpdatedAt: testutil.MustTime(entry.UpdatedAt),
	}

	entries := make([]interface{}, 1)
	entries[0] = entry

	s.collectionStorage.EXPECT().QueryItemByMaxSortKey(
		gomock.Any(),
		traceByHashPartitionKeyFixture,
		traceByHashSortKeyFixture,
		models.TraceByHashDDBEntry{},
		nil,
	).Return(&collection.QueryItemsResult{Items: entries, LastEvaluatedKey: nil}, nil)

	trace, err := s.storage.GetTraceByHash(ctx, traceByHashTagFixture, traceByHashHashFixture, traceByHashSequenceFixture)
	require.NoError(err)
	require.Equal(expectedTraces, trace)
}

func (s *TraceByHashStorageTestSuite) TestGetTrace_NoBlockTime() {
	require := testutil.Require(s.T())

	ctx := context.Background()
	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)
	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithData(compressed),
		Height: traceByHashHeightFixture,
		Hash:   traceByHashHashFixture,
	}

	expectedTraces := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Sequence:  traceByHashSequenceFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Data:      data,
		UpdatedAt: testutil.MustTime(entry.UpdatedAt),
	}

	entries := make([]interface{}, 1)
	entries[0] = entry

	s.collectionStorage.EXPECT().QueryItemByMaxSortKey(
		gomock.Any(),
		traceByHashPartitionKeyFixture,
		traceByHashSortKeyFixture,
		models.TraceByHashDDBEntry{},
		nil,
	).Return(&collection.QueryItemsResult{Items: entries, LastEvaluatedKey: nil}, nil)

	trace, err := s.storage.GetTraceByHash(ctx, traceByHashTagFixture, traceByHashHashFixture, traceByHashSequenceFixture)
	require.NoError(err)
	require.Equal(expectedTraces, trace)
}

func (s *TraceByHashStorageTestSuite) TestGetTrace_InvalidBlockTime() {
	require := testutil.Require(s.T())

	ctx := context.Background()
	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)
	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithData(compressed),
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		BlockTime: "invalid",
	}

	entries := make([]interface{}, 1)
	entries[0] = entry

	s.collectionStorage.EXPECT().QueryItemByMaxSortKey(
		gomock.Any(),
		traceByHashPartitionKeyFixture,
		traceByHashSortKeyFixture,
		models.TraceByHashDDBEntry{},
		nil,
	).Return(&collection.QueryItemsResult{Items: entries, LastEvaluatedKey: nil}, nil)

	trace, err := s.storage.GetTraceByHash(ctx, traceByHashTagFixture, traceByHashHashFixture, traceByHashSequenceFixture)
	require.Error(err)
	require.Nil(trace)
}

func (s *TraceByHashStorageTestSuite) TestGetTrace_LargeFile() {
	require := testutil.Require(s.T())

	ctx := context.Background()
	data := []byte("asdqwd")
	compressed, err := compression.Compress(data, compression.CompressionGzip)
	require.NoError(err)
	objectKey := "s3-location"
	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithObjectKey(objectKey),
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	expectedTrace := &xapi.Trace{
		Tag:       traceByHashTagFixture,
		Sequence:  traceByHashSequenceFixture,
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		Data:      data,
		BlockTime: s.blockTimeFixture,
		UpdatedAt: testutil.MustTime(entry.UpdatedAt),
	}

	entries := make([]interface{}, 1)
	entries[0] = entry

	s.collectionStorage.EXPECT().QueryItemByMaxSortKey(
		gomock.Any(),
		traceByHashPartitionKeyFixture,
		traceByHashSortKeyFixture,
		models.TraceByHashDDBEntry{},
		nil,
	).Return(&collection.QueryItemsResult{Items: entries, LastEvaluatedKey: nil}, nil)

	s.collectionStorage.EXPECT().
		DownloadFromBlobStorage(gomock.Any(), entry).
		DoAndReturn(func(ctx context.Context, entry collection.Item) error {
			err := entry.SetData(compressed)
			require.NoError(err)
			return nil
		})

	trace, err := s.storage.GetTraceByHash(ctx, traceByHashTagFixture, traceByHashHashFixture, traceByHashSequenceFixture)
	require.NoError(err)
	require.Equal(expectedTrace, trace)
}

func (s *TraceByHashStorageTestSuite) TestGetTrace_LargeFileDownloadFailure() {
	require := testutil.Require(s.T())

	ctx := context.Background()
	objectKey := "s3-location"
	entry := &models.TraceByHashDDBEntry{
		BaseItem: collection.NewBaseItem(
			traceByHashPartitionKeyFixture,
			traceByHashSortKeyFixture,
			traceByHashTagFixture,
		).WithObjectKey(objectKey),
		Height:    traceByHashHeightFixture,
		Hash:      traceByHashHashFixture,
		BlockTime: traceByHashBlockTimeFixture,
	}

	entries := make([]interface{}, 1)
	entries[0] = entry

	s.collectionStorage.EXPECT().QueryItemByMaxSortKey(
		gomock.Any(),
		traceByHashPartitionKeyFixture,
		traceByHashSortKeyFixture,
		models.TraceByHashDDBEntry{},
		nil,
	).Return(&collection.QueryItemsResult{Items: entries, LastEvaluatedKey: nil}, nil)

	s.collectionStorage.EXPECT().DownloadFromBlobStorage(gomock.Any(), entry).
		Return(xerrors.Errorf("failed to download"))

	trace, err := s.storage.GetTraceByHash(ctx, traceByHashTagFixture, traceByHashHashFixture, traceByHashSequenceFixture)
	require.Error(err)
	require.Nil(trace)
}
