...
func (c *callImpl) Instrument(ctx context.Context, operation OperationFn, opts ...InstrumentOption) error {
	...
	if err != nil {
		if c.filter != nil && c.filter(err) {
			c.onSuccess(logger, span, finishTime, err)
		} else {
	...
	c.onSuccess(logger, span, finishTime, nil)
	return nil
}
...
func (c *callImpl) onSuccess(logger *zap.Logger, span tracer.Span, finishTime time.Time, err error) {
	c.success.Inc(1)
	logger.Debug(c.loggerMsg, zap.Error(err))
